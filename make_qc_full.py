"""
Generate QC outputs for the full 800-stack registration:
...
output_dir = Path("./results/2026-03-02-01_registered_3d")
results_prefix = "full800"
overlay_movie_path = f"./results/{results_prefix}_overlay.mp4"
registered_movie_path = f"./results/{results_prefix}_registered.mp4"
stills_dir = Path(f"./results/{results_prefix}_stills")
registered_stills_dir = Path(f"./results/{results_prefix}_registered_stills")
gif_path = f"./results/{results_prefix}_rotating_mip.gif"
...
mip_timepoint = 399  # middle of full dataset
frame_vol = tifffile.imread(frame_files[mip_timepoint]).astype(np.float32)

# Anisotropy correction: Z=1.0µm, XY=0.772µm at 2x downsample
z_scale = 1.0 / 0.772
